<style>
    ...
    #dvProdutos .input-group .input-group-addon {
        background-color: #dd4b39;
        border-color: #dd4b39;
    }
    #dvProdutos .input-group .input-group-addon a {
        color: #fff;
    }
</style>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group" id="selectLojaDestino">
                    <label for="status">Estoque Origem</label>
                    <select name="cod_loja_origem" id="cod_loja_origem" data-placeholder="Selecione a Loja" class="form-control select2 select2-hidden-accessible input-tip selectLojaDestino" style="width:100%;" tabindex="-1" onchange="validaTransferencia();$('#btnPesquisaInserirProdutoTrasnferencia').removeClass('disabled')" aria-hidden="true">
                        ...
                        <div class="box-tools">
                            <div class="input-group input-group-sm" style="width: 250px;">
                                ...
                                <div class="input-group-btn">
                                    <button id="btnPesquisaInserirProdutoTrasnferencia" type="button" class="btn btn-default disabled"><i id="iconPesquisaInserirProdutoTrasnferencia" class="fa fa-search"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-group" id="selectLojaDestino">
                    <label for="status">Loja/Estoque destino</label>
...
<script>
    ...
    function validaTransferencia() {
        ...
        $('#btnConfirmarTransferencia').attr('disabled', !valido);
    ...
    function buscaProdutoByCodONome() {
        ...
        $.ajax({
            ...
            data: {
                ...
                cod_loja_origem: $('#cod_loja_origem').val()
            ...
            if (json != []) {
                ...
                if (obj.dados) {
                    ...
                    $.each(obj.dados, function(idx, arr) {
                        ...
                        input_focus = 'qtd_transferir_' + arr.code;
                        
                        html += '<tr>';
                        html += '<td>' + arr.code + '</td>';
                        html += '<td>' + arr.name + '</td>';
                        html += '<td>' + arr.quantity + '</td>';
                        html += '<td><input type="number" id="qtd_transferir_' + arr.code + '" onkeypress="enter_qtd_transferir(this, event)"></td>';
                        html += '<td><a href="javascript:void(0);" title="" id="btn_qtd_transferir_' + arr.code + '" class="tip btn btn-success btn-xs" onclick="insereProdutoPesquisado(false,false,' + arr.id + ',\'' + arr.code + '\',\'' + arr.name + '\',\'' + arr.quantity + '\', document.getElementById(\'qtd_transferir_' + arr.code + '\').value)" data-original-title="Transferir Estoque"><i class="fa fa-long-arrow-right"></i></a></td>';
                        html += '</tr>';
    ...
    function salvaProdutosSessao() {
        ...
        var v = $('#pesquisaInserirProdutoTrasnferencia').val('');
                console.log(v);
                var f = $('#pesquisaInserirProdutoTrasnferencia').focus();
                console.log(v);
    }
                ...
                if (json != 'null') {
                    ...
                    if (obj.dados) {
                        ...
                        $.each(obj.dados, function(idx, arr) {
                            ...
                            $('#dvProdutos table tbody').append(getHtmlInserirItem(arr.idx, arr.id_produto, arr.cod_produto, arr.nome_produto, arr.qtd_atual, arr.qtd_transferir));
...
<script>
        ...
        $.ajax({
            ...
            if (json != []) {
                ...
                $.each(obj.dados, function(idx, arr) {
                    ...
                    if (arr.tipo == 'DEPOSITO') {
                        $('#cod_loja_origem').append($('<option>', {
                            value: arr.cod,
    ...
    function enter_qtd_transferir(elm, ev) {
        if (ev.charCode !== 13) {
            return;
        }
        ...
        confirmarTransferencia = false;
    }
    
    var confirmarTransferencia = false;
    
    window.addEventListener('load', function() {
        $('#pesquisaInserirProdutoTrasnferencia').keypress(function (ev) {
            if (ev.charCode === 13) {
                confirmarTransferencia = false;
                buscaProdutoByCodONome();
            }
