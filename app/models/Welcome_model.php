...
class Welcome_model extends CI_Model
{
    ...
    public function getChartData($user_id = NULL) {
        ...
        $myQuery = "SELECT S.month, S.total, S.tax, S.discount
            FROM (	SELECT	date_format(date, '%Y-%m') Month, SUM(grand_total) total, SUM(total_tax) tax, SUM(total_discount) discount
                FROM ".$this->db->dbprefix('sales')."
                WHERE date >= date_sub( now( ) , INTERVAL 12 MONTH ) AND status = 'paid'";
        if($user_id) {
