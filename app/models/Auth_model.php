...
class Auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select('u.*, g.name AS group_name')
                ->from($this->tables['users'] . ' AS u, groups g')
                ->where($this->identity_column, $this->db->escape_str($identity))
                ->where('g.id', 'u.group_id', false)
                ->limit(1)
                ->get();
    ...
    public function set_session($user) {
        ...
        $query = $this->db->get_where('permissoes', ['usuario_id' => $user->id]);

        foreach ($query->result() as $row) {
            
            $parts = explode('/', $row->rota);
            
            if (isset($parts[1]) && !isset($permissoes[$parts[1]])) {
                $permissoes[$parts[1]] = 1;
            }
            
            $permissoes[$row->rota] = 1;
        }
