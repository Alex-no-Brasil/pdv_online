...
class Sales extends MY_Controller {
    ...
    function get_sales() {
        ...
        $this->datatables->select("$sales.id as sid, $sales.date, $sales.unique_key, $sales.seq_id, $cust.name as cname, $sel.name as sname, $sales.status, grand_total");
        $this->datatables->from($sales);

        $cod_loja = $this->input->post('cod_loja');

        if (!empty($cod_loja)) {
            $this->datatables->where("$sales.cod_loja", $cod_loja);
        }
        ...
        if (empty($cod_loja)) {
            ...
            $data['footer'] = $this->sales_model->valorTotalLoja($cod_loja, $date_start, $date_end);
        }
    ...
    public function canceled() {
        ...
        $unique_key = $this->input->post('key');

        $this->sales_model->canceled($unique_key);
    }
    ...
    function vendas_geral() {

        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['page_title'] = lang('sales');
        $bc = array(array('link' => '#', 'page' => lang('Vendas Geral')));
        $meta = array('page_title' => lang('Vendas Geral'), 'bc' => $bc);
        ...
        $this->page_construct('sales/vendas_geral', $this->data, $meta);
    }
    
    function get_vendas_geral() {
        ...
        $this->datatables->select("$sales.id as sid, $sales.id, $sales.date, $sales.date as hora, $sales.unique_key, $cust.name as cname, $sales.total_items, grand_total, $sales.status, $sel.name as sname");
        $this->datatables->from($sales);
